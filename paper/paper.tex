...
to compare the latest version with one of the older versions.
%
...
This means that the user can use the same string ``\tettt{git RE 2}''
for getting the README.md file of the day before yesterday.
The command is ``$ git ... '@{2 days ago}''.
Although such command options are very difficult to remember,
users can do the same thing only by saying RE and 2.
...
Text input systems for various languages are used widely in the world.
They are called ``????''
