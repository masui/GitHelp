...
\item Introduction of a ``translation paradigm''
...
We have implemented the ``GitHtlp'' system that can translate
user' intentions into a correct \texttt{git} commands
using the \textsf{ExpandHelp} framework.

\paragraph{Entering a command}

We assume that a user is using \texttt{bash} for developing a software system.
...
conparison and invoke the following command,

\begin{quotation}
  \verb|$ git diff README.md|
\end{quotation}

but nothing happens, because this is not the right command
to compare the latest version with older version.
%
More experienced user might use a command like the following,
but it does not show anything either, because
\verb|HEAD^^| means...
...
Instead, the user can type \texttt{Cmd-Ctrl-G} to invoke
\textsf{GitHelp}
after typing
\texttt{\$ git RE 2}.
...
\begin{figure}[b]
  ...
  \caption{Invoking GitHelp.}
  \label{bash1}
\end{figure}

Here, various command strings and descriptions are listed,
one of which says ``.....'',
