...
Help documents used in ExpandHelp are provided as a set of data entries
...
which are easily understandable to users,
...
\begin{quote}
  description: \\
  \texttt{Find a (Chinese|Korean|Vietnamese) restaurant \\  in (San Francisco|Chicago|New York) \\  on Yelp.com} \\
  command: \\
  \texttt{open 'http://www.yelp.com/search?\\  find\_loc=\$2\&find\_desc=\$1'}
\end{quote}

The first regular expression, the description part,
is for generating texts like
\sf{Find a Chinese restaurant in San Francisco}, 
\sf{Find a Chinese restaurant in Chicago}, etc.,
and the second part, command part,
is used to generate a command for accessing \texttt{Yelp.com}.
% Here, $1 and $2 corresponds to the matched pattern in the regular expression.
In the first example, \sf{Chinese} and \sf{San Francisco}
match the first and second parentheses of the regular expression, and
these values are assigned to \texttt{\$1} and \texttt{\$2}
for the command part, generating a URL
\texttt{http://www.yelp. com/search?find\_loc=San\%20Francisco\&find\_desc= Chinese}.

When the matched pattern like \sf{San Francisco}
cannot be directly used in the command,
parameters for the command can be put after the description string,
separated by a TAB character.
For example, when a number like ``\texttt{29247}'' should be used
for checking the
weather of San Francisco, we can describe the help entry like this:

\begin{quote}\small
 description: \\
 \texttt{Check the weather of (San Francisco{\textbackslash}t29247| New York{\textbackslash}t23164)} \\
 command: \\
 \texttt{open 'http://local.msn.com/ten-day.aspx?\\eid=\$1'}
\end{quote}

Here, selecting a menu entry \sf{Check the weather of San Francisco}
will produce a command
\texttt{open 'http:{\slash}{\slash}local. msn.com{\slash}ten-day.aspx?eid=29247'}.
...
generate a state transition diagram which represents all the strings represented
...
% and we will describe the details of the algorithm later in this section.

\subsection*{Phase 1: Generating a state transition diagram from a regular expression}

Regular expressions are widely used for finding a pattern in a text string
in modern programming languages and in the Unix environment.
In ExpandHelp, we use a regular expression for
generating various patterns of descriptions in a short form.
For example, we use a short regular expression
\tt{(remove|delete|erase) (data|file)}
...
% \includegraphics[height=12pt]{figures/startnode.pdf}
% depicts the start node.
...
\subsection*{Phase 2: Generating texts from a state transition diagram}

Using the state transition diagram shown in Figure \ref{statemachine1},
ExpandHelp generates all the strings represented in the regular expressions,
...
Starting at the start node (\raisebox{-2pt}{\includegraphics[height=10pt,bb=0 0 40 40]{figures/startnode.pdf}})
...
After visiting the initial node \raisebox{-2pt}{\includegraphics[height=10pt,bb=0 0 40 40]{figures/startnode.pdf}},
the system generates a string \sf{Find a} and proceeds to the next node.
In the second generation,
the system can add \sf{Chinese}, \sf{Korean}, and \sf{Vietnamese}
to \sf{Find a}, generating
\sf{Find a Chinese},
\sf{Find a Korean}, etc.
...
from a regular expression like ``\texttt{(0|1)+}'' which represents infinite length of
strings consisting of \texttt{0}s and \texttt{1}s, so the generation should be
...
Using this strategy, the ``8-Queen'' problem can be solved simply by
...
controlling the generation part from the testing part is usually effective.
...
We call it the ``\textit{generate-and-filter}'' strategy,
and implemented the algorithm in ExpandHelp.
Unlike simple puzzle problems where
conditions are strict and finding one solution is enough,
our goal is to find help description strings which
fits the query pattern while allowing errors.
For this goal,
