var data = {
	...
	"$ Compare (#{files}) with the one from (#{numbers}) hours ago {8}",
	"$ 「(#{files})」ファイルが(#{numbers})時間前から(変化した|変わった)ところを(#{display}) {8}",
	"% git diff HEAD '@{#{$2} hours ago}' #{$1} {8}",
	"$ (#{numbers})分前の「(#{files})」ファイルと現在の(もの|バージョン)を比較する {8}",
	"% git diff HEAD '@{#{$1} minutes ago}' #{$2} {8}",
	"$ Compare (#{files}) with the one from (#{numbers}) days ago {8}",
	...
	"% git diff $(git rev-list -n 1 HEAD -- #{$2})^^ -- #{$2} {8}",
	"$ Compare (#{files}) with the one from 2 versions before {8}",
	...
	"$ Show (#{files}) from (#{numbers}) versions before {24}",
	"% git show HEAD~#{$2}:#{$1} {24}",
	...
	"$ Show the edit history of (#{files}) from (#{numbers}) minites ago {8}",
	...
	" $ Show the edit history of (#{files}) from (#{numbers}) hours ago {8}",
	...
	"$ Show the edit history of (#{files}) from (#{numbers}) days ago {37}",
